...
function secondsToTime(remainingSeconds: number) {
  ...
  let hours = Math.floor(integerSeconds / (60 * 60));

  let divisor_for_minutes = integerSeconds % (60 * 60);
...
export function Timer({
  ...
  const [timeLeftMs, setTimeLeft] = useState(
    startTimestamp + durationMs - Date.now()
  );
